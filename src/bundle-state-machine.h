...
class BundleStateMachineImpl
 ...
 public:
  ...
  //
  // This method must be called only in response to the BundleReady callback,
  // and must be called before calling Continue.
  boost::shared_ptr<AnnotatedBundleData> RetrieveGeneratedBundle() const;

  // After BundleReadyCallback is invoked, Continue must be called to re-start
  // the state machine to resume processing any partial snapshot that may still
  // remain unprocessed. Calling Continue will discard the previously generated
  // bundle, so RetrieveGeneratedBundle must be called prior to calling
  // Continue.
  void Continue();
 ...
 protected:
  ...
  PE_STATE_MACHINE_DEFINE_EVENT(ContinueAfterBundleRetrieved);
